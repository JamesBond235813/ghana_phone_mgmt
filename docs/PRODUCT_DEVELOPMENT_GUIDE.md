...
> 项目目录：`/Users/jackbond/Desktop/ghana_phone_mgmt`
...
| 深圳仓综合作业员 | 深圳收购、验收、装托盘、装箱 | 一人连续创建采购入库、逐台验收、装托盘和装箱封存 | 修改已确认单据、跨地点操作、确认国际发运 |
| 深圳发运员 | 出口发运 | 创建/确认发运单、物流资料 | 伪造收货结果、修改已发运清单 |
| 加纳管理处综合作业员 | 到货验收、仓库拣货/调拨、退回分诊、维修建单和维修 QA | 承接到货、逐台验收、门店拣货、退回分诊、建立维修单和独立 QA | 国际发运；代替维修技师接单或填写维修结果 |
...
当前演示库的规范账号为 `szops`（深圳仓综合作业员）、`szdispatch`（深圳发运员）和
`ghops`（加纳管理处综合作业员）；早期按工序拆开的演示账号仅保留为停用历史记录，
不能登录或重新授权。维修技师 `repairtech` 仍独立负责接单和填写维修结果，之后由
`ghops` 在维修区执行 QA。
